...
import type {
  AgentId, AgentState, Approval, ChatMsg, MemEntry, MemKind, Packet,
  PlanTracker, Settings, TermLine, Toast,
} from "./types";
...
function makeAgent(id: AgentId): AgentState {
  ...
  const lines: TermLine[] = (WARM_LINES[id] || []).map((l, i) => ({
    id: uid(), kind: l.kind, text: l.text, ts: seedTs + i * 1200,
  }));
  return {
    id,
    status: id === "cricket" ? "away" : "idle",
    statusText: id === "cricket" ? "at the coffee machine" : "on the floor",
    pos: id === "cricket" ? { x: 82, y: 74 } : { ...def.desk },
    bubble: null,
    stream: null,
    lines,
    current: null,
    ...
    idleTimer: 4000 + Math.random() * 8000,
  };
}
...
export interface OfficeState {
  ...
  selected: AgentId;
  inspectorTab: "terminal" | "queue" | "memory";
  chat: ChatMsg[];
  packets: Packet[];
  approvals: Approval[];
  memory: MemEntry[];
  ...
  select: (id: AgentId, tab?: "terminal" | "queue" | "memory") => void;
...
export const useOffice = create<OfficeState>()(
  persist(
    (set, get) => ({
      ...
      pushToast: (kind, text) =>
        set((s) => ({ toasts: [...s.toasts.slice(-3), { id: uid(), kind, text }] })),
      dismissToast: (id) => set((s) => ({ toasts: s.toasts.filter((t) => t.id !== id) })),
      addMemory: (scope, kind, text, by) =>
        set((s) => ({
          memory: [...s.memory, { id: uid(), scope, kind, text, ts: Date.now(), by }],
        })),
    ...
    }),
    {
      ...
      partialize: (s) => ({
        chat: s.chat,
        memory: s.memory,
        settings: { ...s.settings, paused: false },
        stats: s.stats,
        everBooted: s.everBooted,
        approvals: s.approvals.slice(-12),
      }),
    },
  ),
);

/** Non-reactive helper used by the engine. */
